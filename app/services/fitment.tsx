...
const upgradeOptions = [
  {
    ...
    description: 'ECU tuning, air intake, and exhaust system upgrades',
    price: 49999,
    icon: Gauge
  },
  {
    ...
    description: 'Body kit, alloy wheels, and LED lighting package',
    price: 39999,
    icon: Sparkles
  },
  {
    ...
    description: 'Paint protection film, ceramic coating, and window tinting',
    price: 29999,
    icon: Shield
  }
];

export default function FitmentScreen() {
  ...
  useEffect(() => {
    if (step === 3) {
      getProviders('fitment', location.latitude, location.longitude);
    }
  }, [step]);

  const handleNext = async () => {
    if (step === 3 && selectedVehicle && selectedOption) {
      const selectedProduct = upgradeOptions.find(opt => opt.id === selectedOption);
      ...
      <ScrollView style={styles.scrollView}>
        <ProgressTracker 
          steps={[
            ...
            { label: 'Choose Upgrade', completed: step > 2, current: step === 2 },
        ...
        {step === 2 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Choose Upgrade Package</Text>
            {upgradeOptions.map((option) => (
              ...
              >
                <View style={styles.cardContent}>
                  <option.icon size={24} color={colors.secondary} />
